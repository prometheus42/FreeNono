...
/**
 * Handles a chat service via NonoWeb. By default the main chat channel of FreeNono is used.
 * Otherwise a chat identifier has to be given when instantiating this handler.
 * <p>
 *
 *
...
public class ChatHandler {
    ...
    private final NonoWebConnection connection;
     ...
     */
    protected ChatHandler(final NonoWebConnection connection, final String chatChannelId) {

        if (connection == null) {
            throw new IllegalArgumentException("Argument connection should not be null.");
        }
        if (chatChannelId == null) {
            throw new IllegalArgumentException("Argument chatChannelId should not be null.");
        }
    ...
    public final void sendMessage(final String message) {
        ...
        if (message != null) {
            ...
            final ChatParser parser = new ChatParser(message, connection);
            if (parser.isCommand() && !parser.isWrongCommand()) {
                connection.sendChatMessage(chatChannel, parser.getReturnMessage());
            }
        }
    }
     ...
     */
    public final void receiveMessageBy(final MessageListener<String> messageListener) {
    ...
    /**
     * Closes this chat. After calling this method no more messages are received by NonoWeb.
     */
    ...
    /**
     * Sets chat name for current player. This name is used whenever messages are sent by this
     * handler.
     *
     * @param chatName
