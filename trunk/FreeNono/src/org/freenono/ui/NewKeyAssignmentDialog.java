...
public class NewKeyAssignmentDialog extends JDialog {
    ...
    /**
     * Defines a hash set with all accepted key codes. The key codes are initialized by a static
     * initializer block. This field is used by the <code>isReservedKey()</code> method.
     */
    private static final Set<Integer> ACCEPTED_KEY_CODES = new HashSet<>();
    static {
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F1);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F2);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F3);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F4);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F5);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F6);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F7);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F8);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F9);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F10);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F11);
        ACCEPTED_KEY_CODES.add(KeyEvent.VK_F12);
    }
    ...
    private void initialize() {
        ...
        final JLabel hint = new JLabel(Messages.getString("OptionsUI.UserKeyPrompt"), JLabel.CENTER);
        c.gridx = 0;
        c.gridy = 0;
        c.gridheight = 1;
        c.gridwidth = 1;
    ...
    private void addListener() {
        ...
        addKeyListener(new KeyListener() {
            ...
            public void keyReleased(final KeyEvent e) {
                ...
                if (!keycodeAlreadyUsed(e.getKeyCode()) && !isReservedKey(e.getKeyCode())) {

                    newKeyCode = e.getKeyCode();
                    logger.debug("New key code for control " + control + ": " + e.getKeyCode());
    ...
    private boolean keycodeAlreadyUsed(final int enteredKeyCode) {
        ...
        if (enteredKeyCode != newKeyCode) {
            ...
            if (enteredKeyCode == settings.getKeyCodeForControl(Control.MOVE_LEFT)) {
                return true;
            }
            if (enteredKeyCode == settings.getKeyCodeForControl(Control.MOVE_RIGHT)) {
                return true;
            }
            if (enteredKeyCode == settings.getKeyCodeForControl(Control.MOVE_UP)) {
                return true;
            }
            if (enteredKeyCode == settings.getKeyCodeForControl(Control.MOVE_DOWN)) {
                return true;
            }
            if (enteredKeyCode == settings.getKeyCodeForControl(Control.MARK_FIELD)) {
                return true;
            }
            if (enteredKeyCode == settings.getKeyCodeForControl(Control.OCCUPY_FIELD)) {
                return true;
            }
        }
    ...
    private boolean isReservedKey(final int enteredKeyCode) {
        ...
        return ACCEPTED_KEY_CODES.contains(enteredKeyCode);
    }
