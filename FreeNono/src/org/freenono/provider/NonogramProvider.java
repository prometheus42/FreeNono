...
/**
 * Provides a nonogram inside a course inside a collection from some source. Lazy loading can be
 * implemented because the nonogram is only fetched when a property is requested or method
 * fetchNonogram() is called.
 *
...
public interface NonogramProvider {
    ...
    /**
     * Returns a string object representing this nonogram. Most implementations should return the
     * name of the nonogram.
     *
    ...
    /**
     * Returns a <code>NonogramProvider</code> for the next nonogram in the course of this nonogram.
     * Which nonogram is chosen isn't determined but each implementation should respect level
     * attribute of nonograms.
     *
     * @return NonogramProvider for next nonogram or <code>null</code> if no next nonogram exists
     */
    NonogramProvider getNextNonogram();

    /**
     * Returns a <code>NonogramProvider</code> for the previous nonogram in the course of this
     * nonogram. Which nonogram is chosen isn't determined but each implementation should respect
     * 'level' attribute of nonograms.
     *
     * @return NonogramProvider for previous nonogram or <code>null</code> if no previous nonogram
     *         exists
     */
